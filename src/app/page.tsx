...
export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <p className="text-ck-text-secondary text-sm tracking-[2px] mb-16">KNOW THE HOUSE. BEAT THE ODDS.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-xl">
        {TOOLS.map(({ href, label, desc }) => (
          ...
          >
            <div className="text-base font-bold tracking-[2px] text-ck-text-primary group-hover:text-ck-accent mb-2">{label}</div>
            <div className="text-sm text-ck-text-secondary leading-relaxed">{desc}</div>
          </Link>
        ))}
      </div>
    </div>
